 ...
 * DorisStreamLoad
 **/
public class CDCDorisStreamLoad implements Serializable {

    private static final Logger LOG = LoggerFactory.getLogger(CDCDorisStreamLoad.class);
    ...
    public CDCDorisStreamLoad(String hostPort, String db, String tbl, String user, String passwd, Properties streamLoadProp, ResolvedSchema schema) {
    ...
    private HttpURLConnection getConnection(String urlStr, String label) throws IOException {
        ...
        // 批量删除功能
        List<String> columns = this.schema.getColumns().stream().map(column -> column.getName()).collect(Collectors.toList());
        String cols = StringUtils.join(columns.iterator(), ",")+",delete_flag";
        conn.addRequestProperty("columns",cols);
        conn.addRequestProperty("delete","delete_flag=1");
